...
@csrf_exempt
@method_awaited(["GET", "POST"])
def GetAllAdd(request):
    db = Database.get()
    if request.method == "GET":
        data = db.run("""
                SELECT IdBanque as id, CONCAT(Utilisateur.Nom, ' ', Utilisateur.Prenom) AS enseignant , TypeCours.Nom AS 'type', CONCAT(Ressource.Libelle,' - ',Ressource.Nom) AS libelle, Couleur.CouleurHexa AS 'style'
                FROM Banque
