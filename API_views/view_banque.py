...
def by_id(request, code: int):
    ...
    if request.method == "GET":
        sql = """
        SELECT IdBanque as id, IdBanque as idBanque, CONCAT(Utilisateur.Prenom, ' ', Utilisateur.Nom) AS enseignant ,  Banque.Duree as duree, TypeCours.Nom AS 'type', CONCAT(Ressource.Libelle,' - ',Ressource.Nom) AS libelle, Couleur.CouleurHexa AS 'style', CONCAT(Ressource.Libelle,' - ',Ressource.Abreviation) as abreviation
        FROM Banque
        LEFT JOIN Utilisateur
        ON Banque.IdUtilisateur = Utilisateur.IdUtilisateur
        LEFT JOIN TypeCours
